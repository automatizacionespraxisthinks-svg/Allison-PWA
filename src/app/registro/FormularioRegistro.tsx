...
export function FormularioRegistro({
  ...
  hayGoogle: boolean;
  /** El mensaje con el que devuelve Google a quien no tenía cuenta. */
  ...
  /**
   * Con Google no hay formulario entre el botón y la cuenta, así que el
   * consentimiento se deja anotado ANTES de ir a Google: el servidor lo
   * guarda en una cookie firmada y lo exige al volver
   * (src/lib/entrada-google.ts). Sin las casillas no se sale de aquí.
   */
  async function continuarConGoogle() {
    if (!aceptaLegal || !declaraEdad) {
      setError("Marca las dos casillas de abajo antes de continuar con Google.");
      return;
    }
    setError(null);
    setConGoogle(true);

    const r = await pedir("/api/registro/google", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ nivel, aceptaLegal, declaraEdad }),
    });
    const d = r ? await r.json().catch(() => ({})) : {};
    if (!r?.ok) {
      setError(d.error ?? "No pudimos preparar la entrada con Google. Intenta de nuevo.");
      setConGoogle(false);
      return;
    }

    // Se va a Google y vuelve a /practicar con la cuenta creada.
    await signIn("google", { redirectTo: "/practicar" });
    setConGoogle(false);
  }
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col justify-center gap-6 px-6 py-10">
      ...
      {avisoGoogle && (
        <p role="status" className="rounded-xl bg-primario-suave p-3 text-sm text-primario">
          {avisoGoogle}
        </p>
      )}
      ...
      {hayGoogle && (
        <>
          <button
            ...
            disabled={conGoogle}
            className="flex items-center justify-center gap-3 rounded-xl border border-borde bg-superficie px-4 py-3 font-medium transition hover:bg-superficie-2 disabled:opacity-50"
          >
      ...
      <form onSubmit={crearCuenta} className="flex flex-col gap-4">
        ...
        {/*
          ...
          antemano no es válido: la ley exige que sea expreso. Valen
          para las dos formas de crear la cuenta: el formulario y Google.
        */}
